...
def get_combinator_context(context):
...
def step_combinator_module_available(context):
...
def step_validate_minimum_and_maximum(context, value, min_val, max_val):
...
def step_validate_even_or_div5(context, value):
    cc = get_combinator_context(context)
...
def step_validate_not_even(context, value):
    cc = get_combinator_context(context)
    is_even = predicate(lambda x: x % 2 == 0, 'Value must be even')
...
def step_validate_complex_combinators(context, value, min_val):
    cc = get_combinator_context(context)
    min_validator = minimum(min_val)
    is_even = predicate(lambda x: x % 2 == 0, 'Value must be even')
    is_div_by_5 = predicate(lambda x: x % 5 == 0, 'Value must be divisible by 5')
...
def step_validate_operator_and(context, value):
    cc = get_combinator_context(context)
...
def step_combinator_result_is_success_with_int_value(context, expected):
    cc = get_combinator_context(context)
...
def step_combinator_result_is_failure_with_error(context, expected_error):
    cc = get_combinator_context(context)
...
def step_combinator_result_is_failure(context):
    cc = get_combinator_context(context)
    assert cc.result.is_nothing(), f'Expected failure but got success: {cc.result}'
