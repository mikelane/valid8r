...
from valid8r.core.parsers import parse_bool, parse_int, parse_list, parse_str
...
def step_env_module_exists(context: Context) -> None:  # noqa: ARG001
    ...
    try:
        ...
        assert False, 'valid8r.integrations.env module does not exist'
...
@given('environment variable {var_name} is not set')
def step_env_var_not_set_for_required(context: Context, var_name: str) -> None:
    """Ensure an environment variable is not set."""
    if not hasattr(context, 'test_environ'):
        context.test_environ = {}
...
def step_schema_with_list_parser(context: Context, field_name: str) -> None:
    ...
    list_parser = lambda text: parse_list(text, element_parser=parse_str, delimiter=',')

    context.schema = EnvSchema(fields={field_name: EnvField(parser=list_parser)})


@when('I call load_env_config(schema, prefix="{prefix}")')
def step_call_load_env_config_with_prefix(context: Context, prefix: str) -> None:
    """Call load_env_config with the given prefix."""
    context.result = context.load_env_config(
        schema=context.schema, prefix=prefix, environ=getattr(context, 'test_environ', {})
    )


@when('I call load_env_config(schema, prefix="{prefix}", delimiter="{delimiter}")')
def step_call_load_env_config_with_delimiter(context: Context, prefix: str, delimiter: str) -> None:
...
@when('I call load_env_config(schema, prefix="{prefix}", list_separator="{list_separator}")')
def step_call_load_env_config_with_list_separator(context: Context, prefix: str, list_separator: str) -> None:
...
def step_assert_success_nested_dict(context: Context) -> None:
    """Assert that the result is a Success with the expected nested dictionary."""
